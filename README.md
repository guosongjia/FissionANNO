...
- `intra_genus_HGT_from_<sp>` — likely acquired by horizontal transfer from a congeneric species (target hit is more similar to donor species than to the reference, passes the absolute identity threshold (> 90%), and exceeds the SOG-derived pairwise cutoff between reference and donor species members)
